...
##  Function
##-----------------------------------------------------------------------------
def segment(eyeim, minrad, multiplier, sigma, virt, horiz, eyelashes_thres=80, use_multiprocess=True):
    """
    Description:
    ...
    rowp, colp, rp = searchInnerBound(eyeim, minrad, multiplier)
    ...
    if use_multiprocess:
        ...
        p_top = mp.Process(
            ...
            args=(imsz, imageiris, irl, icl, rowp, rp, sigma, virt, horiz, ret_top),
        )
        p_bot = mp.Process(
            target=findBottomEyelid,
            args=(imsz, imageiris, rowp, rp, irl, icl, sigma, virt, horiz, ret_bot),
        )
        p_top.start()
        p_bot.start()
        p_top.join()
    ...
    # If not use_multiprocess
        ...
        mask_top = findTopEyelid(imsz, imageiris, irl, icl, rowp, sigma, virt, horiz, rp)
        mask_bot = findBottomEyelid(imsz, imageiris, rowp, rp, irl, icl, sigma, virt, horiz)
...
# ------------------------------------------------------------------------------
def findTopEyelid(imsz, imageiris, irl, icl, rowp, rp, sigma, virt, horiz, ret_top=None):
    """
    Description:
    ...
    lines = findline(topeyelid, sigma, virt, horiz)
...
# ------------------------------------------------------------------------------
def findBottomEyelid(imsz, imageiris, rowp, rp, irl, icl, sigma, virt, horiz, ret_bot=None):
    """
    Description:
    ...
    lines = findline(bottomeyelid, sigma, virt, horiz)
    mask = np.zeros(imsz, dtype=float)
